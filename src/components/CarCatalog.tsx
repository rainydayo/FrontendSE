...
export default async function CarCatalog({carJson}:{carJson:RestaurantJson}) {
    ...
    return (
        ...
        <div data-test="catalog-div" style={{margin:"20px", display:"flex", flexDirection:"row" , flexWrap:"wrap", justifyContent:"space-around",alignContent:"space-around"}}>
                {
                    carJsonReady.data.map((carItem:RestaurantItem)=>(
                        <Link data-test={`restaurant-link-${carItem.name}`} href={`/restaurant/${carItem._id}`} className="w-[100%] sm:w-[50%] md:w-[30%] lg:w-[25%] p-2 sm:p-4 md:p-4 lg:p-8">
